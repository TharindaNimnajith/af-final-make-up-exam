...
class BookBody extends Component {
  ...
  getBookDetails = () => {
    let {displayLoader, hideLoader, match} = this.props;

    displayLoader();

    axios.get(`${bookAPI}/${match.params.id}`)
      .then((result) => {
            ...
            (Object.keys(this.state.book).length > 0) ? (
              <Fragment>
                <div className={"col-md-3 mt-5"}>
                  <img
                    src={`${fileGetAPI}/${get(this.state, "book.image", "")}`}
                    alt="..."
                ...
                <div className={"col-md-9 mt-5"}>
                  <div style={{marginLeft: "50px"}}>
                    <h1 style={{fontWeight: "700"}}>{get(this.state, "book.name", "")}</h1>

                    <dl className={"row mt-4"}>
                      <dt className={"col-sm-3"}>Author</dt>
                      <dd className={"col-sm-9"}>{get(this.state, "book.author", "")}</dd>

                      <dt className={"col-sm-3"}>Category</dt>
                      <dd className={"col-sm-9"}>{get(this.state, "book.category.name", "")}</dd>

                      <dt className={"col-sm-3"}>Published Date</dt>
                      <dd className={"col-sm-9"}>{dateToString(get(this.state, "book.published_date", ""))}</dd>

                      <dt className={"col-sm-3"}>Description</dt>
                      <dd className={"col-sm-9"}>{get(this.state, "book.description", "")}</dd>
                    </dl>
                  </div>
                </div>
              </Fragment>
